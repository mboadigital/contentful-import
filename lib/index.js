...
import { getFileFromAWS } from './tasks/push-to-space/assets'
...
export default async function runContentfulImport (params) {
  const log = []
  let options = parseOptions(params, false)
  // Setup AWS S3 client
  ...
  params.content = await getFileFromAWS(s3Client, '//export.json', bucket, dataDir, true)
  options = parseOptions(params, true)
